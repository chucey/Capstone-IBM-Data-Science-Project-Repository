...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'All':
        fig = px.pie(spacex_df, values='class', names='Launch Site',hole=0.5,
        title='All sites Successful Launchest')
        return fig
        ...
        fig = px.pie(filtered_df,
              ...
              color_discrete_map={1: '#8FBC8F', 0: '#DC143C'}, title = entered_site + ' Successful Launch rate' )
        return fig
...
def get_scatter_plot(entered_site, payload_mass):
    ...
    if entered_site == 'All':
        ...
        filtered_df = spacex_df[spacex_df['Launch Site']==entered_site]
        refined_scat_df = filtered_df[(filtered_df['Payload Mass (kg)'] >= payload_mass[0])
            & (spacex_df['Payload Mass (kg)'] <= payload_mass[1])]
        scat_plot1 = px.scatter(refined_scat_df, x = 'Payload Mass (kg)', y = 'class',
            color="Booster Version Category", symbol = "Booster Version Category",
            title = "Payload vs class - " + entered_site )
        scat_plot1.update_traces(marker={'size': 15})
        return scat_plot1
